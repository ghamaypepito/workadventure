...
describe("MatrixChatConnection", () => {
    ...
    describe("createDirectRoom", () => {
        it("coalesces concurrent room creation requests for the same user", async () => {
            const mockMatrixClient = {
                isGuest: vi.fn(),
                on: vi.fn(),
                store: { startup: vi.fn() },
            ...
            vi.spyOn(matrixChatConnection, "getDirectRoomFor").mockReturnValue(undefined);

            let resolveCreation: (room: MatrixChatRoom) => void = () => undefined;
            const createdRoom = { id: "canonical" } as unknown as MatrixChatRoom;
            const createOnce = vi.fn().mockReturnValue(
                new Promise((resolve) => {
                    resolveCreation = resolve;
                }),
            );
            matrixChatConnection["createDirectRoomOnce"] = createOnce;

            const first = matrixChatConnection.createDirectRoom("@alice:example.test");
            const second = matrixChatConnection.createDirectRoom("@alice:example.test");
            resolveCreation(createdRoom);

            await expect(first).resolves.toBe(createdRoom);
            await expect(second).resolves.toBe(createdRoom);
            expect(createOnce).toHaveBeenCalledOnce();
        });

        it("prefers a room with message history over a newer empty duplicate", async () => {
            const mockMatrixClient = {
                isGuest: vi.fn(),
                on: vi.fn(),
                store: { startup: vi.fn() },
            ...
            const established = {
                id: "established",
                getMemberIdsSync: () => ["@me:test", "@alice:test"],
                getLatestMessageTimestampSync: () => 100,
                getCreationTimestampSync: () => 10,
                myMembership: readable(KnownMembership.Join),
                type: readable("direct"),
            } as unknown as MatrixChatRoom;
            const emptyDuplicate = {
                id: "empty",
                getMemberIdsSync: () => ["@me:test", "@alice:test"],
                getLatestMessageTimestampSync: () => undefined,
                getCreationTimestampSync: () => 20,
                myMembership: readable(KnownMembership.Join),
                type: readable("direct"),
            } as unknown as MatrixChatRoom;
            matrixChatConnection["roomList"].set(established.id, established);
            matrixChatConnection["roomList"].set(emptyDuplicate.id, emptyDuplicate);

            expect(matrixChatConnection.getDirectRoomFor("@alice:test")).toBe(established);
        });

        it("prefers the oldest room when both duplicates contain message history", async () => {
            const mockMatrixClient = {
                isGuest: vi.fn(),
                on: vi.fn(),
                store: { startup: vi.fn() },
            ...
            const recentlyActive = {
                id: "recently-active",
                getMemberIdsSync: () => ["@me:test", "@alice:test"],
                getLatestMessageTimestampSync: () => 1_000,
                getCreationTimestampSync: () => 20,
                myMembership: readable(KnownMembership.Join),
                type: readable("direct"),
            } as unknown as MatrixChatRoom;
            matrixChatConnection["roomList"].set(oldest.id, oldest);
            matrixChatConnection["roomList"].set(recentlyActive.id, recentlyActive);

            expect(matrixChatConnection.getDirectRoomFor("@alice:test")).toBe(oldest);
        });

        it("does not reuse a two-person room that is not marked as a direct message", async () => {
            const mockMatrixClient = {
                isGuest: vi.fn(),
                on: vi.fn(),
                store: { startup: vi.fn() },
            ...
            matrixChatConnection["roomList"].set(privateGroup.id, privateGroup);

            expect(matrixChatConnection.getDirectRoomFor("@alice:test")).toBeUndefined();
        });
