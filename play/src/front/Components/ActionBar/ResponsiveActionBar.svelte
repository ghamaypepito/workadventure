<script lang="ts">
    ...
    let centerDivWidth: number = $state(0);
    let leftDivWidth: number = $state(0);
    let leftToCenterWidth: number = $state(0);
    ...
    let fullMenuVisible: boolean = $state(false);
...
>
    <div
        {@attach blocker}
        class="bg-[#1c2a41] backdrop-blur rounded-lg pointer-events-auto gap-1 @md/actions:gap-1.5 @xl/actions:gap-2 p-1 @md/actions:p-1.5 @xl/actions:p-2 max-w-6xl mx-auto mb-2"
    >
