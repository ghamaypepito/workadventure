import type { DeepPartial } from "../DeepPartial";
import type { Translation } from "../i18n-types";

const chat: DeepPartial<Translation["chat"]> = {
    intro: "Aqui está seu histórico de bate-papo:",
    adventurers: "Usuários",
    onlineUsers: "Usuário online",
    getCloserTitle: "Aproxime-se de alguém",
    noRoomOpen: "Abrir uma conversa",
    noRoomOpenDescription:
        "Nada para exibir ainda. Inicie um chat ou aproxime-se de alguém para ver suas mensagens aparecerem aqui.",
    getCloserDesc:
        "Quando você se aproxima de alguém no espaço virtual, uma bolha de conversa abre automaticamente, permitindo que você converse diretamente como se estivesse cara a cara.",
    noMessage: "Nenhuma mensagem enviada ainda!",
    beFirst: "Seja o primeiro e envie uma mensagem",
    enter: "Digite sua mensagem...",
    menu: {
        visitCard: "Cartão de visita",
        addFriend: "Adicionar amigo",
    },
    loader: "Carregando...",
    typing: "está digitando...",
    users: "Usuários",
    chat: "Chat",
    userList: {
        disconnected: "Não conectado ao mundo",
        isHere: "Está neste mapa",
        inAnotherMap: "Em outro mapa",
        in: "Em ",
        teleport: "Teletransportar",
        search: "Apenas procure!",
        TalkTo: "Falar com",
        teleporting: "Teletransportando...",
        businessCard: "Cartão de Visita",
        sendMessage: "Enviar Mensagem",
        follow: "Localizar",
        invite: "Convidar",
    },
    accept: "Aceitar",
    decline: "Recusar",
    meetingInvitation: {
        title: "{name} convida você a participar da reunião",
        invitationDeclined: "Seu convite foi recusado por {name}",
        invitationAccepted: "Seu convite foi aceito por {name}",
        defaultResponderName: "O usuário",
        limitReached: "Você enviou muitos convites para reuniões. Tente novamente mais tarde.",
    },
    socialSignal: {
        wave: "Wave",
        ping: "Ping",
        wavedToYou: "{name} waved to you",
        wantsToTalk: "{name} wants to talk to you",
        pingedYou: "{name} pinged you",
        youWavedAt: "You waved at {name}",
        youPinged: "You pinged {name}",
        dismiss: "Dismiss",
    },
    imagePreview: {
        close: "Fechar",
        label: "Pré-visualização da imagem",
        loading: "Carregando imagem...",
        loadError: "Não foi possível carregar esta pré-visualização da imagem.",
        openInNewTab: "Abrir em nova guia",
        notAvailable: "Imagem não disponível.",
        imageActions: "Ações da imagem",
    },
    join: "Entrar",
    search: "Pesquisar",
    closeSearch: "Fechar pesquisa",
    closeChat: "Fechar chat",
    connecting: "Conectando ao servidor...",
    waitingInit: "Aguardando inicialização do servidor...",
    waitingData: "Aguardando dados do usuário...",
    searchUser: "Procurar usuário, mapa, etc...",
    searchChat: "Procurar canal, mensagem, etc...",
    people: "Mensagem direta",
    rooms: "Conversas em grupo",
    addNew: "Adicionar novo: ",
    invitations: "Convites",
    roomInvitation: {
        notification: "Você foi convidado para a sala {roomName}",
        notificationTitle: "Novo convite",
        unknownRoom: "Uma sala",
    },
    suggestedRooms: "Salas sugeridas",
    ignoreSuggestedRoom: "Ignorar",
    joinableRooms: "Discussões disponíveis",
    joinableRoomsError: "Erro ao carregar discussões, clique para tentar novamente",
    availableRooms: "Salas disponíveis",
    proximity: "Chat de Proximidade",
    role: {
        adminShort: "Admin",
        admin: "Administrador",
        member: "Membro",
        visitor: "Visitante",
    },
    status: {
        online: "Online",
        away: "Ausente",
        unavailable: "Indisponível",
        back_in_a_moment: "Volto em instantes",
        do_not_disturb: "Não perturbe",
        busy: "Ocupado",
        meeting: "Em uma reunião",
        megaphone: "Usando o megafone",
    },
    logIn: "Entrar",
    signIn: "Registre-se ou faça login para aproveitar todos os recursos do chat!",
    invite: "Convidar",
    roomEmpty: "Esta sala está vazia, convide um colega ou amigo para se juntar a você!",
    userOnline: "usuário online",
    usersOnline: "usuários online",
    open: "Abrir",
    me: "Eu",
    you: "Você",
    ban: {
        title: "Banir",
        content: "Banir usuário {userName} do mundo em execução. Isso pode ser cancelado pela administração.",
        ban: "Banir este usuário",
    },
    loading: "Carregando",
    loadingUsers: "Carregando os usuários...",
    load: "Carregar",
    rankUp: "Promover",
    rankDown: "Rebaixar",
    reinit: "Reinicializar",
    enterText: "Digite uma mensagem...",
    timeLine: {
        title: "Sua Linha do Tempo",
        open: "Abra seu histórico da linha do tempo!",
        description: "Histórico de mensagens e eventos",
        newDiscussion: "Nova discussão com {userNames}",
        incoming: "{userName} entrou na discussão",
        outcoming: "{userName} saiu da discussão",
        youLeft: "Você saiu da discussão",
        youleftMeetingRoom: "Você saiu da sala de reunião",
        youJoinedMeetingRoom: "Você entrou na sala de reunião",
        today: "Hoje",
        yesterday: "Ontem",
        retry: "Tentar novamente",
    },
    form: {
        placeholder: "Digite sua mensagem...",
        typing: " digitando...",
        application: {
            klaxoon: {
                title: "Klaxoon",
                description: "Enviar klaxoon incorporado no chat!",
            },
            youtube: {
                title: "YouTube",
                description: "Enviar vídeo do youtube incorporado no chat!",
            },
            googleDocs: {
                title: "Google Docs",
                description: "Enviar google docs incorporado no chat!",
            },
            googleSlides: {
                title: "Google Slides",
                description: "Enviar google slides incorporado no chat!",
            },
            googleSheets: {
                title: "Google Sheets",
                description: "Enviar google sheets incorporado no chat!",
            },
            eraser: {
                title: "Borracha",
                description: "Enviar borracha incorporada no chat!",
            },
            googleDrive: {
                title: "Google Drive",
                description: "Enviar um arquivo do google drive no chat!",
            },
            excalidraw: {
                title: "Excalidraw",
                description:
                    "Enviar um quadro branco virtual de código aberto com estilo desenhado à mão. Colaborativo e criptografado de ponta a ponta.",
            },
            cards: {
                title: "Cards",
                description: "Enviar um cartão no chat!",
            },
            tldraw: {
                title: "Tldraw",
                description: "Enviar quadro branco online Tldraw.",
            },
            weblink: {
                error: "Por favor, digite uma URL válida",
            },
        },
    },
    notification: {
        discussion: "quer discutir com você",
        message: "envia uma mensagem",
        forum: "no fórum",
        in: "em",
    },
    see: "Ver",
    show: "Mostrar",
    less: "menos",
    more: "mais",
    sendBack: "Enviar de volta",
    delete: "Excluir",
    messageDeleted: "Mensagem excluída",
    emoji: {
        icon: "Ícone para abrir ou fechar popup de emoji selecionado",
        search: "Procurar emojis...",
        categories: {
            recents: "Emojis Recentes",
            smileys: "Carinhas e Emoções",
            people: "Pessoas e Corpo",
            animals: "Animais e Natureza",
            food: "Comida e Bebida",
            activities: "Atividades",
            travel: "Viagem e Lugares",
            objects: "Objetos",
            symbols: "Símbolos",
            flags: "Bandeiras",
            custom: "Personalizado",
        },
        notFound: "Nenhum emoji encontrado",
    },
    said: "disse:",
    reply: "Responder",
    replyTo: "Responder para",
    thread: {
        panelTitle: "Todas as threads",
        tabLabel: "Threads",
        empty: "Ainda não há threads",
        defaultPreview: "Thread",
        replySingular: "1 resposta",
        replyPlural: "{count} respostas",
        openThreadButtonTitle: "Abrir thread",
        unreadAriaLabel: "{count} não lidas",
        partialHistory: "O histórico do tópico está disponível apenas parcialmente nesta sala.",
        degradedListWarning: "{count} tópico(s) estão sem o início.",
        degradedBadge: "Início indisponível",
        rootUnavailable: "O início desta conversa não está disponível. Você ainda pode ler e responder.",
        openError: "Não foi possível abrir este tópico.",
    },
    react: "Reagir",
    copy: "Copiar",
    copied: "Copiado!",
    poll: {
        title: "Enquete",
        closed: "Enquete encerrada.",
        vote: "Votar",
        updateVote: "Atualizar voto",
        removeVote: "Remover voto",
        submitError: "Não foi possível enviar seu voto. Tente novamente.",
        endError: "Não foi possível encerrar a enquete. Tente novamente.",
        deleteError: "Não foi possível excluir a enquete. Tente novamente.",
        participants: "{count} voto(s)",
        multiSelect: "Até {count} opções",
        spoiledVotes: "{count} voto(s) inválido(s)",
        resultsAfterVote: "Os resultados aparecerão depois que você votar.",
        resultsWhenClosed: "Os resultados serão exibidos quando a enquete for encerrada.",
        kind: {
            open: "Aberta",
            closed: "Fechada",
        },
        end: {
            cta: "Encerrar enquete",
            generic: "Enquete encerrada.",
            withWinner: "Enquete encerrada. Resposta em primeiro lugar: {answer}",
        },
        create: {
            title: "Criar enquete",
            description: "Crie uma enquete nesta sala.",
            question: "Pergunta",
            questionPlaceholder: "Faça uma pergunta",
            answers: "Respostas",
            addAnswer: "Adicionar resposta",
            answerPlaceholder: "Resposta {index}",
            answerCount: "{count}/{max} respostas prontas",
            visibility: "Visibilidade",
            openDescription: "Os resultados ficam visíveis depois que você votar.",
            closedDescription: "Os resultados permanecem ocultos até o encerramento da enquete.",
            cancel: "Cancelar",
            submit: "Criar enquete",
            validationError: "Digite uma pergunta e pelo menos 2 respostas.",
            submitError: "Não foi possível criar a enquete. Tente novamente.",
        },
    },
    question: {
        title: "Perguntas",
        description: "Abrir perguntas da reunião",
        ask: "Fazer uma pergunta",
        placeholder: "Escreva sua pergunta...",
        send: "Enviar",
        toAnswer: "Para responder ({count})",
        answeredFilter: "Respondidas ({count})",
        answered: "Respondida",
        openStatus: "abertas",
        emptyOpen: "Ainda não há perguntas para responder.",
        emptyAnswered: "Ainda não há perguntas respondidas.",
        upvote: "Votar na pergunta",
        markAnswered: "Marcar como respondida",
        unknownAuthor: "Desconhecido",
        notification: "Pergunta: {question}",
    },
    file: {
        fileContentNoEmbed: "Conteúdo indisponível para visualização. Por favor, baixe",
        download: "baixar",
        openCoWebsite: "Abrir em co-website",
        loadingAttachment: "Carregando anexo...",
        attachmentDownloadError: "Não foi possível baixar este anexo.",
        attachmentDecryptError: "Não foi possível descriptografar este anexo.",
        copy: "copiar o link",
        tooBig: "{fileName} é muito grande {maxFileSize}.",
        notLogged: "Você precisa estar logado para enviar um arquivo.",
    },
    needRefresh: "Sua conexão expirou, você precisa atualizar a página para reconectar ao chat.",
    refresh: "Atualizar",
    upgrade: "Atualizar",
    upgradeToSeeMore: "Atualize para ver mais mensagens",
    disabled: "Este recurso está desabilitado.",
    disabledByAdmin: "Este recurso está desabilitado pelo administrador.",
    anAdmin: "um administrador",
    messageDeletedByYou: "Você excluiu esta mensagem",
    messageEdited: "Modificado",
    messageEditedError: "Não foi possível editar a mensagem. Tente novamente.",
    waiting: "Aguardando",
    nothingToDisplay: "Nada para exibir",
    showMore: "Mostrar {number} mais",
    showLess: "Mostrar menos",
    addRoomToFolderError: "Impossível adicionar a sala à pasta",
    createRoom: {
        title: "Criar nova sala",
        rootTitle: "Criar nova discussão",
        name: "Nome",
        visibility: {
            label: "Visibilidade",
            private: "Privada",
            privateDescription: "Apenas convidados poderão encontrar e entrar na sala.",
            public: "Pública",
            publicDescription: "Qualquer pessoa pode encontrar e entrar na sala.",
            restricted: "Membro da pasta",
            restrictedDescription: "Visível apenas para membros da pasta",
        },
        e2eEncryption: {
            label: "Ativar criptografia de ponta a ponta",
            description: "Você não poderá desativá-la mais tarde.",
        },
        suggested: "Sala sugerida",
        suggestedDescription: "Esta sala será sugerida aos usuários que estão no mesmo espaço.",
        users: "Usuários",
        historyVisibility: {
            label: "Quem pode ler o histórico?",
            world_readable: "Qualquer pessoa",
            joined: "Apenas membros (desde que entraram)",
            invited: "Apenas membros (desde que foram convidados)",
        },
        buttons: {
            edit: "Editar",
            create: "Criar",
            cancel: "Cancelar",
        },
        error: "Erro na criação da sala",
        loadingCreation: "Criação da sala em andamento",
        creationSuccessNotification: "Sala criada",
        restrictedDescription: "Esta sala será visível apenas para membros da pasta pai: {folderName}",
    },
    createFolder: {
        title: "Criar nova pasta",
        name: "Nome",
        visibility: {
            label: "Visibilidade",
            private: "Privada",
            privateDescription: "Apenas convidados poderão encontrar e entrar na sala.",
            public: "Pública",
            publicDescription: "Qualquer pessoa pode encontrar e entrar na sala.",
            restricted: "Membro da pasta",
            restrictedDescription: "Visível apenas para membros da pasta",
        },
        description: {
            label: "Descrição",
            placeholder: "Descrição",
        },
        e2eEncryption: {
            label: "Ativar criptografia de ponta a ponta",
            description: "Você não poderá desativá-la mais tarde.",
        },
        users: "Usuários",
        historyVisibility: {
            label: "Quem pode ler o histórico?",
            world_readable: "Qualquer pessoa",
            joined: "Apenas membros (desde que entraram)",
            invited: "Apenas membros (desde que foram convidados)",
        },
        buttons: {
            create: "Criar",
            cancel: "Cancelar",
        },
        error: "Erro na criação da sala",
        loadingCreation: "Criação da sala em andamento",
        creationSuccessNotification: "Sala criada",
    },
    manageRoomUsers: {
        roomOption: "Participantes",
        error: "Não foi possível enviar convites",
        title: "Gerenciar participantes",
        invitations: "Convites",
        participants: "Participantes",
        join: "Entrou",
        invite: "Convidado",
        ban: "Banido",
        kick: "Expulso",
        leave: "Saiu",
        roomID: "ID da Sala: {roomId}",
        membership: "Associação",
        permissionLevel: "Funções",
        actions: "Ações",
        buttons: {
            sendInvitations: "Enviar convites",
            cancel: "Cancelar",
            ban: "Banir",
            kick: "Expulsar",
            unban: "Desbanir",
            invite: "Convidar",
        },
        roles: {
            USER: "Usuário",
            ADMIN: "Admin",
            MODERATOR: "Moderador",
        },
        sendInvitationsSuccessNotification: "Convites enviados com sucesso",
    },
    roomPanel: {
        toggleOpen: "Abrir painel da sala",
        toggleClose: "Fechar painel da sala",
        back: "Voltar para a visão geral da sala",
        status: {
            partialHistory: "Histórico parcial",
            pollDetailsUnavailable: "Os resultados detalhados desta enquete não estão disponíveis no momento.",
            retry: "Tentar novamente",
        },
        sections: {
            threads: "Tópicos",
            polls: "Enquetes",
            participants: "Participantes",
            settings: "Configurações",
        },
        home: {
            directRoom: "Sala direta",
            groupRoom: "Sala de grupo",
            encrypted: "Criptografada",
            notEncrypted: "Não criptografada",
            readOnly: "Somente leitura",
        },
        settings: {
            roomType: "Tipo de sala",
            encryption: "Criptografia",
            joinRule: "Acesso",
            restrictedTo: "Restrita a",
            readOnly: "Estas configurações são somente leitura aqui.",
            unknown: "Desconhecido",
            joinRules: {
                public: "Pública",
                invite: "Somente por convite",
                restricted: "Restrita",
                knock: "Solicitar acesso",
            },
        },
        pollsEmpty: "Ainda não há enquetes",
        participantsEmpty: "Ainda não há participantes",
        pollWinner: "Resposta na liderança: {answer}",
        threadsLoadError: "Não foi possível carregar os tópicos. Tente novamente.",
        pollsLoadError: "Não foi possível carregar as enquetes. Tente novamente.",
        pollsPartial: "Algumas enquetes não puderam carregar seus resultados detalhados.",
        pollFocusError: "Não foi possível abrir esta enquete na linha do tempo.",
    },
    roomMenu: {
        leaveRoom: {
            label: "Sair da sala",
            notification: "Você saiu da sala",
        },
        muteRoom: "Silenciar Sala",
        unmuteRoom: "Reativar Som da Sala",
    },
    folderMenu: {
        leaveFolder: {
            label: "Sair da Pasta",
            notification: "Você saiu da pasta",
        },
    },
    e2ee: {
        configure: "Configurar",
        encryptionNotConfigured: "Criptografia não configurada",
        createRecoveryKey: {
            title: "Criação de chave de recuperação do chat",
            description:
                "Para usar criptografia de ponta a ponta no chat, você precisa criar uma chave de recuperação. Por favor, digite sua frase de recuperação abaixo, uma chave de recuperação será criada.",
            privateKeyDescription:
                "Esta é sua chave de recuperação, salve-a em algum lugar para recuperar discussões criptografadas após fazer logout.",
            error: "Algo deu errado em generateRecoveryKeyFromPassphrase",
            buttons: {
                generate: "Gerar",
                continue: "Continuar",
                cancel: "Cancelar",
            },
        },
        interactiveAuth: {
            title: "Criptografia de ponta a ponta para chat",
            description:
                "Por razões de segurança, uma chave deve ser enviada para nosso servidor para verificar sua identidade. Ao confirmar sua identidade, você armazenará a chave, que permite ler mensagens criptografadas do WA e outros clientes.",
            instruction: "Certifique-se de completar a conexão SSO antes de clicar no botão Finalizar.",
            buttons: {
                cancel: "Cancelar",
                continueSSO: "Continuar com SSO",
                finish: "Finalizar",
            },
        },
        accessSecretStorage: {
            title: "Verificação de sessão do chat",
            description:
                "Para verificar sua sessão e recuperar mensagens criptografadas, você precisa inserir sua chave de recuperação ou frase secreta.",
            passphrase: "Frase secreta",
            recoveryKey: "Chave de recuperação",
            placeholder: "Digite sua",
            buttons: {
                cancel: "Cancelar",
                usePassphrase: "Usar frase de recuperação em vez disso",
                useRecoveryKey: "Usar chave de recuperação em vez disso",
                confirm: "Confirmar",
            },
        },
    },
    connectionError: "Chat não disponível",
    unknownError: "Erro desconhecido",
    matrixUserSelect: {
        userNotFound: "Usuário não encontrado",
        failedToLoadUsers: "Falha ao carregar usuários",
    },
    a11y: {
        menuActions: "Menu de ações",
        unreadCount: "{count} não lidas",
        userAvatar: "Avatar do usuário",
        applicationIcon: "Aplicativo",
    },
    requiresLoginForChatModal: {
        title: "Faça login para desbloquear o chat!",
        content_1: "Para aproveitar totalmente este recurso de chat, você precisa estar logado em sua conta.",
        content_2: "Fazer login dá acesso a todas as opções e permite continuar sem interrupção.",
        content_3: " Por favor, faça login para prosseguir com a experiência.",
    },
    verificationEmojiDialog: {
        confirmation: "Eles coincidem",
        mismatch: "Eles não coincidem",
        titleVerifyThisDevice: "Verificar seu dispositivo",
        titleVerifyOtherDevice: "Verificar outro dispositivo",
        description: "Confirme que os emojis abaixo são exibidos em ambos os dispositivos e na mesma ordem:",
        waitForOtherDevice: "Aguardando o outro dispositivo...",
        waitOtherDeviceConfirmation: "aguardando confirmação do outro dispositivo...",
        understood: "Entendido",
        error: "Erro durante o processo de validação…",
    },
    chooseDeviceVerificationMethodModal: {
        title: "Verificar dispositivo",
        description: "Escolha o método de verificação",
        withOtherDevice: "Verificar com outro dispositivo",
        withPassphrase: "Verificar com frase secreta de segurança",
    },
    askStartVerificationModal: {
        title: "Verificação necessária",
        accept: "Verificar a sessão",
        ignore: "Ignorar",
        from: "de",
    },
    requiresLoginForChat: "Faça login para acessar as mensagens do chat",
    emojis: {
        Dog: "Cachorro",
        Cat: "Gato",
        Lion: "Leão",
        Horse: "Cavalo",
        Unicorn: "Unicórnio",
        Pig: "Porco",
        Elephant: "Elefante",
        Rabbit: "Coelho",
        Panda: "Panda",
        Rooster: "Galo",
        Penguin: "Pinguim",
        Turtle: "Tartaruga",
        Fish: "Peixe",
        Octopus: "Polvo",
        Butterfly: "Borboleta",
        Flower: "Flor",
        Tree: "Árvore",
        Cactus: "Cacto",
        Mushroom: "Cogumelo",
        Globe: "Globo",
        Moon: "Lua",
        Cloud: "Nuvem",
        Fire: "Fogo",
        Banana: "Banana",
        Apple: "Maçã",
        Strawberry: "Morango",
        Corn: "Milho",
        Pizza: "Pizza",
        Cake: "Bolo",
        Heart: "Coração",
        Smiley: "Sorriso",
        Robot: "Robô",
        Hat: "Chapéu",
        Glasses: "Óculos",
        Spanner: "Chave inglesa",
        Santa: "Papai Noel",
        "Thumbs Up": "Joinha",
        Umbrella: "Guarda-chuva",
        Hourglass: "Ampulheta",
        Clock: "Relógio",
        Gift: "Presente",
        "Light Bulb": "Lâmpada",
        Book: "Livro",
        Pencil: "Lápis",
        Paperclip: "Clipe de papel",
        Scissors: "Tesoura",
        Lock: "Cadeado",
        Key: "Chave",
        Hammer: "Martelo",
        Telephone: "Telefone",
        Flag: "Bandeira",
        Train: "Trem",
        Bicycle: "Bicicleta",
        Aeroplane: "Avião",
        Rocket: "Foguete",
        Trophy: "Troféu",
        Ball: "Bola",
        Guitar: "Guitarra",
        Trumpet: "Trombeta",
        Bell: "Sino",
        Anchor: "Âncora",
        Headphones: "Fones de ouvido",
        Folder: "Pasta",
        Pin: "Alfinete",
        unknownLabel: "Rótulo Desconhecido",
    },
    fileAttachment: {
        title: "Anexo de arquivo",
        description: "Enviar e compartilhar um arquivo",
        featureComingSoon: "Em breve!",
    },
    loginTokenError: "Ocorreu um erro. Por favor, tente reconectar",
    reconnect: "Reconectar",
    failedToJoinRoom: "Falha ao entrar na sala",
    failedToLeaveRoom: "Falha ao sair da sala",
    refreshChat: "Atualizar Chat",
    dismiss: "Dispensar",
    whoops: "Ops! algo deu errado",
    failedToOpenRoom: "Falha ao abrir sala: {roomId}",
    failedToSendAttachments: "Falha ao enviar anexos. Tente novamente.",
    refreshChatError: "Falha ao atualizar chat",
    discord: {
        chatBand: {
            configure: "Configurar",
            open: "Abrir",
            dismiss: "Dispensar",
            discordNotConfigured: "Conectar ao Discord",
        },
    },
    remoteUserNotConnected: "Usuário não autenticado. Não é possível enviar mensagem.",
    matrixSettings: {
        title: "Matrix & WorkAdventure",
        subtitle: "Sua identidade de chat e perfil Matrix",
        close: "Fechar",
        identitySection: "Identidade",
        matrixUserId: "ID de usuário Matrix",
        homeserver: "Homeserver",
        copy: "Copiar",
        copied: "Copiado",
        profileSection: "Perfil Matrix",
        accountDataSection: "WorkAdventure (no jogo)",
        displayName: "Nome de exibição",
        avatar: "Avatar (MXC)",
        localName: "Nome local no jogo",
        waDisplayName: "Nome WA (fr.workadventure.wa_display_name)",
        waAvatar: "WA avatar (fr.workadventure.wa_avatar)",
        needsSyncHint:
            "Seu nome local ou WOKA pode ser diferente do seu perfil Matrix. Use Sincronizar para atualizar.",
        upToDateHint:
            "Seu perfil Matrix corresponde ao nome local no jogo e ao WOKA (ou não há nada para sincronizar).",
        syncButton: "Sincronizar perfil Matrix (nome + imagem WOKA)",
        syncing: "Sincronizando…",
        removeProfileImagesButton: "Remover imagens de perfil",
        removingProfileImages: "Removendo…",
        removeProfileImagesHint:
            "Remove a foto do seu perfil Matrix. Seu WOKA no jogo não muda; use Sincronizar para publicá-lo novamente.",
        loadError: "Não foi possível carregar as configurações Matrix.",
        notConnected: "Matrix não está conectado ou você é um convidado.",
        clearSuggestedRoomsHint:
            "Limpa a lista de salas Matrix que você ocultou das listas sugeridas ou disponíveis para entrar (armazenada neste navegador).",
        clearSuggestedRoomsButton: "Redefinir salas sugeridas ocultas",
        publishWokaToMatrixProfileHint:
            "Define o nome de exibição e o avatar do seu perfil Matrix a partir do seu nome no jogo e do WOKA (visível em clientes Matrix como Element).",
        publishWokaToMatrixProfileButton: "Aplicar nome e imagem WOKA ao perfil Matrix",
        publishWokaToMatrixProfilePublishing: "Atualizando perfil…",
        publishWokaToMatrixProfileConfirmWarning:
            "Isso substitui o nome de exibição e o avatar do seu perfil Matrix no servidor. Qualquer nome ou foto definidos em outro lugar (por exemplo, no Element) será substituído. Continue apenas se quiser que seu nome WorkAdventure e seu WOKA se tornem seu perfil Matrix público.",
        publishWokaToMatrixProfileCancelButton: "Cancelar",
        publishWokaToMatrixProfileConfirmActionButton: "Substituir e aplicar",
        actionDone: "Concluído",
    },
    matrixPeerProfileDebug: {
        menuItemTitle: "Perfil Matrix",
        menuItemDebugTag: "debug",
        title: "Perfil Matrix",
        subtitle:
            "Visualização somente leitura do perfil Matrix deste usuário. Exibida apenas quando DEBUG_MODE está ativo.",
        loadError: "Não foi possível carregar o perfil Matrix deste usuário.",
    },
};

export default chat;
